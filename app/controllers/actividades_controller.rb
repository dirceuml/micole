class ActividadesController < ApplicationController
  ...
  def update
    ...
    validaok = 1
    if !params[:actividad][:estado].nil?
      if params[:actividad][:estado].to_i == 2 and estado_anterior == 1 and @actividad.alcance_colegio == 0
        if ActividadSeccion.find_by_actividad_id(params[:id]).nil?
          validaok = 0
        end
      end
    end
    ...
    if validaok == 0
      flash[:notice] = 'Seleccione las secciones que participan en la actividad'
      redirect_to(:actividad) and return
    else
