...
#  has_many :personas_vinculadas
  ...
  scope :inasistencia_fecha, lambda { |fecha| joins(:anio_escolar).where("anios_escolares.activo = 1 and anios_alumnos.estado = 1 and anios_alumnos.id not in (Select anio_alumno_id from asistencias where to_char(fecha_hora, 'yyyymmdd') = ?)", fecha.strftime('%Y%m%d'))}
  ...
  scope :asistencia_fecha_notificarinasistencia, lambda { |fecha, notificarinasistencia| joins(:anio_escolar => :colegio).where("anios_escolares.activo = 1 and colegios.notificar_inasistencia = ? and anios_alumnos.estado = 1 and anios_alumnos.id in (Select anio_alumno_id from asistencias where to_char(fecha_hora, 'yyyymmdd') = ?)", notificarinasistencia, fecha.strftime('%Y%m%d'))}
  scope :inasistencia_anio_escolar_seccion_fecha, lambda { |anioescolar, seccion, fecha| where("anio_escolar_id = ? and seccion_id = ? and estado = 1 and id not in (Select anio_alumno_id from asistencias where to_char(fecha_hora, 'yyyymmdd') = ?)", anioescolar, seccion, fecha.strftime('%Y%m%d'))}
