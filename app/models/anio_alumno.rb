...
#  has_many :personas_vinculadas
  ...
  scope :asistencia_fecha, lambda { |fecha| joins(:anio_escolar).where("anios_escolares.activo = 1 and anios_alumnos.id in (Select anio_alumno_id from asistencias where to_char(fecha_hora, 'dd/mm/yyyy') = ?)", fecha.strftime('%d/%m/%Y'))}
